	...
	<script type="text/javascript">
		$(function () {
			...
			var table = $('#allNotificationsTable').DataTable({
				...
				"order": [[ 0, "desc" ]],
				language: {
					"emptyTable": "<div><img id='no-results-img' src='{{ URL::asset('img/files/no-notification.png') }}'><br>{{ __('There are no emails yet') }}</div>",
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						...
						$.ajax({
							...
							method: 'post',
							url: '<?php echo env('BASE_URL_PUBLIC');?>admin/email/newsletter/delete',
							data: formData,
							processData: false,
							contentType: false,
