	...
	<script type="text/javascript">
		$(function () {
			...
			let table = $('#allTemplates').DataTable({
				...
				"order": [[3, "asc"]],
				language: {
					"emptyTable": "<div><img id='no-results-img' src='{{ URL::asset('img/files/no-result.png') }}'><br>{{ __('No files added yet') }}</div>",
				...
				processing: true,
				serverSide: true,
				ajax: "{{ route('admin.chat.assistant.files') }}",
				columns: [					
					{
						data: 'name',
						name: 'name',
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						...
						$.ajax({
							...
							method: 'post',
							url: '<?php echo env('BASE_URL_PUBLIC');?>admin/chat/assistant/file/delete',
							data: formData,
							processData: false,
							contentType: false,
