		...
		<div class="col-lg-9 col-md-10 col-sm-12">
			<div class="card border-0">	
				<div class="card-header">
					...
					<a href="{{ route('admin.chat.assistant') }}" class="btn btn-cancel ripple" style="margin-left: auto">{{ __('Back to AI Chats') }}</a>
				</div>			
				<div class="card-body pt-5 pb-0 pl-6 pr-6">
					<form class="w-100" action="{{ route('admin.chat.assistant.update', $id->id) }}" method="POST" enctype="multipart/form-data">
						@method('PUT')
						@csrf
						...
						<div class="row">
								...
								<div class="input-box">
									...
									<select class="form-select" name="model">									
										...
										@foreach ($fine_tunes as $fine_tune)
											<option value="{{ $fine_tune->model }}"  @if ($id->model == 'gpt-4-turbo-2024-04-09') selected @endif>{{ $fine_tune->description }}</option>										
										@endforeach	
									</select>
								</div>
							</div>
							...
							<div class="col-md-6 col-sm-12">
								<div class="input-box">
								  ...
								  <select id="chats" name="category" class="form-control">
									<option value="free" @if($id->category == 'free') selected @endif>{{ __('Free Package') }} ({{ __('Access only to Free Chatbots') }})</option>																																											
									<option value="standard" @if($id->category == 'standard') selected @endif> {{ __('Standard Package') }} ({{ __('Access up to Standard Package') }})</option>
									<option value="professional" @if($id->category == 'professional') selected @endif> {{ __('Professional Package') }} ({{ __('Access up to Professional Package') }})</option>
									<option value="premium" @if($id->category == 'premium') selected @endif> {{ __('Premium Package') }} ({{ __('Access up to Premium Package') }})</option>																																																														
								  </select>
								</div>
							  </div>
							...
							<div class="col-lg-4 col-md-6 col-sm-12 mt-2 mb-4 pl-0 text-center">
								<div class="form-group">
								  	<label class="custom-switch">
										<input type="checkbox" name="retrieval" class="custom-switch-input" @if ($id->retrieval) checked @endif>
										<span class="custom-switch-indicator"></span>
										<span class="custom-switch-description">{{ __('Enable File Search Tool') }} <i class="ml-1 text-dark fs-13 fa-solid fa-circle-info" data-tippy-content="{{ __('File Search Tool must be enabled if you want Chat Assistant to consider your uploaded file or let upload a file during chat') }}"></i></span>
								  	</label>
								</div>
							</div>
							...
							<div class="col-lg-4 col-md-6 col-sm-12 mt-2 mb-4 pl-0 text-center">
								<div class="form-group">
								  	<label class="custom-switch">
										<input type="checkbox" name="upload" class="custom-switch-input" @if ($id->upload) checked @endif>
										<span class="custom-switch-indicator"></span>
										<span class="custom-switch-description">{{ __('File Upload by Users') }} <i class="ml-1 text-dark fs-13 fa-solid fa-circle-info" data-tippy-content="{{ __('Let users upload their own files during chatbot usage, uploaded files will expire after 7 days if the chat conversation will be inactive') }}"></i></span>
								  	</label>
								</div>
							</div>
							...
							<div class="col-sm-12 col-md-12">
								<div class="input-box">
								  	<label class="form-label fs-12 font-weight-semibold">{{ __('File Access') }} <span class="text-muted">({{ __('Optional') }})</span> <i class="ml-1 text-dark fs-13 fa-solid fa-circle-info" data-tippy-content="{{ __('File Search Tool must be enabled if you want Chat Assistant to consider your uploaded file or let upload a file during chat') }}"></i></label>
									<div class="input-group file-browser mb-1" id="create-new-chat">									
									...
									<a href="{{ route('admin.chat.assistant.files') }}" class="text-right fs-12 font-weight-semibold">{{ __('View All Included Files to this Chatbot') }}</a>
								</div>
							</div>
						</div>
