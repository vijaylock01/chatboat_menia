				...
				<div class="card-body">		
					...
					<div class="d-flex mb-3">
						<div class="d-flex text-right" id="template-buttons-group">	
							@if (config('settings.integration_feature_user'))
								<div class="btn-group mt-1 publish-action-buttons">
									<button type="button" class="btn btn-primary ripple dropdown-toggle" data-bs-toggle="dropdown" id="export" data-bs-display="static" aria-expanded="false" style="text-transform: none; font-size: 11px">{{ __('Publish') }}</button>
									<div class="dropdown-menu" aria-labelledby="export" data-popper-placement="bottom-start">								
										<a class="dropdown-item" href="{{ route('user.documents.publish', $id) }}"><i class="fa-brands fa-wordpress fs-13 text-muted mr-2"></i> {{ __('Publish to Wordpress') }}</a>
									</div>
								</div>
							@endif										
							...
							<a id="save-button-template" class="template-button" target="{{ $id->id }}" onclick="return saveText(this);" href="#"><i class="fa-solid fa-floppy-disk-pen table-action-buttons table-action-buttons-big delete-action-button" data-tippy-content="{{ __('Save Document') }}"></i></a>				
						</div>					
					</div>
					...
					<div>						
						<div id="template-textarea" class="pl-4 pr-4 show-text-result">		
							@if (!is_null($id->image))
							<div class="text-center mb-5 mt-2">
								<img src="{{ $id->image }}" style="height: 300px; border-radius: 10px;" alt="AI Article Wizard Image">
							</div>								
							@endif				
							<div class="form-control" name="content" rows="25" id="tinymce-editor">{!! $id->result_text !!}</div>
							@error('content')
					...
					<div class="border-0 text-center mb-5 mt-5">
						<a href="{{ route('user.documents') }}" class="btn btn-primary ripple pl-7 pr-7">{{ __('Return') }}</a>
					</div>
				</div>
			</div>
		</div>
...
<script type="text/javascript">
	...
	function saveText(event) {
		...
		$.ajax({
			headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
			method: 'POST',
			url: '<?php echo env('BASE_URL_PUBLIC');?>user/templates/save',
			data: { 'id': event.target, 'text': textarea, 'workbook': workbook, 'title': title},
			success: function (data) {					
				if (data['status'] == 'success') {
