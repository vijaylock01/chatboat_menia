								...
								<div class="flex" id="templates-panel">			
									<div class="row chat-model-box pl-5 pr-5 pt-2 pb-2">
										...
										@if ($chat->type == 'private')
											@if (trim($chat->model) == 'gpt-3.5-turbo-0125')
													...
													<label for="control_01">
														...
														<p class="text-muted" style="position: absolute; bottom: 7px; opacity: 0.8; left: 0; right: 0; font-size: 9px;">
															@if (auth()->user()->gpt_3_turbo_credits == -1)
																{{ __('Unlimited') }}
															@else
																{{ App\Services\HelperService::userAvailableWords() }}
															@endif {{ __('Words Available') }}
														</p>
													</label>
												</div>
											@elseif (trim($chat->model) == 'gpt-4o')
												<div class="col-md-4 col-sm-12">
													...
													<label for="control_10">
														...
														<p class="text-muted" style="position: absolute; bottom: 7px; opacity: 0.8; left: 0; right: 0; font-size: 9px;">
															@if (auth()->user()->gpt_4o_credits == -1)
																{{ __('Unlimited') }}
															@else
																{{ App\Services\HelperService::userAvailableGPT4oWords() }}
															@endif {{ __('Words Available') }}
														</p>
													</label>
												</div>
											@elseif (trim($chat->model) == 'gpt-4-0125-preview')
												<div class="col-md-4 col-sm-12">
													...
													<label for="control_03">
														...
														<p class="text-muted" style="position: absolute; bottom: 7px; opacity: 0.8; left: 0; right: 0; font-size: 9px;">
															@if (auth()->user()->gpt_4_turbo_credits == -1)
																{{ __('Unlimited') }}
															@else
																{{ App\Services\HelperService::userAvailableGPT4TWords() }}
															@endif {{ __('Words Available') }}
														</p>
													</label>
												</div>
											@elseif (trim($chat->model) == 'gpt-4-turbo-2024-04-09')
												<div class="col-md-4 col-sm-12">
														...
														<label for="control_02">
															...
															<p class="text-muted" style="position: absolute; bottom: 7px; opacity: 0.8; left: 0; right: 0; font-size: 9px;">
																@if (auth()->user()->gpt_4_credits == -1)
																	{{ __('Unlimited') }}
																@else
																	{{ App\Services\HelperService::userAvailableGPT4Words() }}
																@endif {{ __('Words Available') }}
															</p>
														</label>
													</div>
																...
																<label for="control_{{ $key }}_f">
																	...
																	<p class="text-muted" style="position: absolute; bottom: 7px; opacity: 0.8; left: 0; right: 0; font-size: 9px;">
																		@if (auth()->user()->fine_tune_credits == -1)
																			{{ __('Unlimited') }}
																		@else
																			{{ App\Services\HelperService::userAvailableFineTuneWords() }}
																		@endif {{ __('Words Available') }}
																	</p>
																</label>
															</div>
				...
				success: function (data) {
					...
					if (data['status'] == 'success') {
						...
						$('.chat-sidebar-messages').prepend(`<div class="chat-sidebar-message selected-message" id=${data['id']}>
								...
								<div class="chat-actions d-flex">
									<a href="#" class="chat-edit fs-12" id="${data['id']}"><i class="fa-sharp fa-solid fa-pen-to-square" data-tippy-content="{{ __('Edit Name') }}"></i></a>
									<a href="#" class="chat-delete fs-12 ml-2"  id="${data['id']}"><i class="fa-sharp fa-solid fa-trash" data-tippy-content="{{ __('Delete Chat') }}"></i></a>
								</div>
							</div>`);
						active_id = data['id'];	
					} else {
