			...
			<div class="card border-0" id="template-input">
				<div class="card-body p-5 pb-0">
					...
					<div class="row">
						<div class="template-view">
							<div class="template-icon mb-2 d-flex">
								<div>
									<i class="fa-solid fa-rss rewriter-icon"></i>
								</div>
								<div>
									<h6 class="mt-2 ml-3 fs-16 number-font">{{ __('AI RSS') }}</h6>
								</div>									
							</div>								
							<div class="template-info">
								<p class="fs-12 text-muted mb-4">{{ __('Generate creative and unique content with your RSS Feed') }}</p>
							</div>
						</div>
					</div>
					...
					<div class="row">
						...
						<div class="col-sm-12">
							<div class="input-box">	
								<h6 class="fs-11 mb-2 font-weight-semibold">{{ __('RSS URL') }}  <span class="text-required"><i class="fa-solid fa-asterisk"></i></span></h6>									
								<div class="form-group relative">						    
									<input class="form-control" id="url" name="url" placeholder="{{ __('Enter RSS URL to Fetch') }}" required />
									<button class="btn btn-primary locale-action-button" type="button" style="right: 8px; border-radius: 0.5rem" id="fetch">{{ __('Fetch RSS') }}</button>
								</div> 
							</div> 
						</div>
						...
						<div class="col-sm-12 mb-5">
							<div id="form-group">
								<h6 class="fs-11 mb-2 font-weight-semibold">{{ __('Fetched RSS Titles') }}</h6>
	...
	$(function () {
		...
		$('#fetch').on('click', function(e) {
			...
			if (url == null || url.value === "") {
				toastr.warning('{{ __("Please provide RSS URL first") }}');
			} else {
				...
				success: function (data) {	
					...
					if (data['status'] == 'error') {
                        ...
                        toastr.success('{{ __('RSS Fetched Successifuly') }}');
					}

					$('#fetch').prop('disabled', false);
					let btn = document.getElementById('fetch');					
					btn.innerHTML = '{{ __('Fetch RSS') }}';
					document.querySelector('#loader-line')?.classList?.add('hidden'); 
				},
				
				error: function(data) {
					$('#fetch').prop('disabled', false);
					$('#fetch').html('{{ __('Fetch RSS') }}'); 
					document.querySelector('#loader-line')?.classList?.add('hidden'); 
					console.log(data)
				}
			});	
			...
			$.ajax({
				...
				success: function (data) {	
					...
					if (data['status'] == 'error') {
						...
						const eventSource = new EventSource( "/user/rss/process?content_id=" + data.id+"&model=" + data.model + "&temperature=" + data.temperature + "&language=" + data.language);
