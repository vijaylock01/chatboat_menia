...
<form id="youtube-form" action="" method="post" enctype="multipart/form-data" class="mt-24"> 		
	@csrf
	<div class="row">	
				...
				<div class="card-body p-5 pb-0">
					...
					<div class="row">
						<div class="template-view">
							<div class="template-icon mb-2 d-flex">
								<div>
									<i class="fa-brands fa-youtube ad-icon"></i>
								</div>
								<div>
									<h6 class="mt-2 ml-3 fs-16 number-font">{{ __('AI Youtube') }}</h6>
								</div>									
							</div>								
							<div class="template-info">
								<p class="fs-12 text-muted mb-4">{{ __('Turn your Youtube videos into Blog posts') }}</p>
							</div>
						</div>
					</div>
					...
					<div class="row">
						...
						<div class="col-sm-12">
							<div class="input-box">	
								...
								<div class="form-group relative">						    
									<input class="form-control" id="url" name="url" placeholder="{{ __('Enter Youtube URL') }}" required />
								</div> 
							</div> 
						</div>
						...
						<div class="col-sm-12 mb-5">
							<div id="form-group">
								<h6 class="fs-11 mb-2 font-weight-semibold">{{ __('Action') }}</h6>
								<select id="action" name="action" class="form-select">
									<option value="post">{{ __('Create a blog post') }}</option>																																							
									<option value="outline">{{ __('Create outlines') }}</option>
									<option value="explain">{{ __('Explain main idea') }}</option>
									<option value="description">{{ __('Create expanded description') }}</option>
									<option value="summarize">{{ __('Summarize the content') }}</option>
									<option value="compare">{{ __('Prepare pros and cons') }}</option>
								</select>
							</div>
						</div>
...
<script type="text/javascript">
	...
	$(function () {
		...
		$('#youtube-form').on('submit', function(e) {
			...
			let check = '{{ $settings->youtube_api }}';
			console.log(check)

			if (check == null || check == '') {
				toastr.warning('{{ __('Please provide Youtube API key first') }}');
				return;
			}
			...
			$.ajax({
				headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
				method: 'POST',
				url: '<?php echo env('BASE_URL_PUBLIC');?>user/youtube/generate',
				data: form.serialize(),
				beforeSend: function() {					
				...
				success: function (data) {	
					...
					if (data['status'] == 'error') {
						...
						const eventSource = new EventSource( "/user/youtube/process?content_id=" + data.id+"&model=" + data.model + "&temperature=" + data.temperature + "&language=" + data.language);
